...
def train_model(df):
    ...
    # Latih model SVR (Support Vector Regressor)
    # SVR memberikan performa terbaik di notebook Anda
    model = SVR()
    model.fit(X, Y)
...
st.write("Aplikasi ini memprediksi harga jual rumah berdasarkan fitur-fitur yang Anda masukkan.")
...
if df is not None:
    ...
    def get_user_input():
        """Mendapatkan input dari pengguna melalui sidebar."""
        input_data = {}
    ...
    # Proses input pengguna agar sesuai format model
    ...
    # 2. Encode kolom kategorikal pengguna
    ...
    st.metric(label="Prediksi Harga", value=f"${prediction[0]:,.2f}")
